...
def _build_review_prompt(
    ...
    parts.append(
        ...
        "## Verdict decision rules (VERY IMPORTANT)\n\n"
        "Use 'publish' when the content meets the voice rules. The content does not "
        "need to be perfect. Mike (the author) will make a final editorial pass "
        "on placeholder sections before posting. Your job is to catch hard quality "
        "failures, not to polish.\n\n"
        "Use 'revise' ONLY for blocking issues. A blocking issue is one of:\n"
        "  (a) a voice rule violation (em-dash, banned phrase, opens a paragraph with 'I')\n"
        "  (b) factually wrong or dangerous code that would break for a reader following along\n"
        "  (c) content that contradicts itself or the steering file\n"
        "  (d) broken Markdown structure that would fail to render correctly\n\n"
        "DO NOT revise for:\n"
        "  - Stylistic preferences (tone, flatness of a single sentence)\n"
        "  - 'Would benefit from' or 'could be improved by' suggestions\n"
        "  - Minor formatting inconsistencies that render fine\n"
        "  - Caveats you think would be nice to add (preview pricing, cross-shell notes)\n"
        "  - Bare URLs or links that render correctly\n"
        "  - Anything inside a <!-- MIKE: --> placeholder. Those are Mike's.\n"
        "  - Missing screenshots or images. Mike verifies these before posting.\n\n"
        "When you feel uncertain, publish. The revision loop is bounded and Mike "
        "prefers a usable draft over a rejected perfect one.\n\n"
        "Use 'spike' only when the content is fundamentally off-topic, incoherent, "
        "or cannot be salvaged by targeted edits.\n\n"
        "If automatic voice rule violations were detected above, the verdict MUST be "
        "'revise' (or 'spike' if the violations are pervasive), and the feedback "
        "MUST name each violation.\n\n"
